...
// OpenStream validates status/Content-Type and transfers Body ownership to a
// bounded SSE parser. Non-200 bodies are consumed, closed, and normalized.
func (mock *mockAdapter) OpenStream(
	ctx context.Context,
	...
	if ctx == nil {
		...
		return nil, errors.New("mock adapter stream context must not be nil")
	}
	if err := ctx.Err(); err != nil {
		if response != nil && response.Body != nil {
			_ = response.Body.Close()
		}
		return nil, fmt.Errorf("mock adapter stream cancelled: %w", err)
	}
	if response == nil || response.Body == nil {
	...
	if response.StatusCode != http.StatusOK {
		...
		return nil, mock.NormalizeError(ctx, response, body)
	}
...
// ctx closes Body to unblock a pending network read.
func (stream *mockStream) Next(ctx context.Context) (adapter.NormalizedChunk, error) {
	if stream == nil {
	...
	if ctx == nil {
		return adapter.NormalizedChunk{}, errors.New("mock stream context must not be nil")
	}
	if err := ctx.Err(); err != nil {
		_ = stream.Close()
...
func (stream *mockStream) parseDataEvent(event sseEvent) error {
	...
	if envelope.Object != "chat.completion.chunk" || envelope.Model != stream.physicalModel {
	...
	for index := range envelope.Choices {
		choice, err := stream.parseStreamChoice(envelope.Choices[index], event.eventType)
		if err != nil {
			return err
		}
		if choice != nil {
	...
	if unknown {
		if err := stream.enqueue(adapter.NormalizedChunk{
			...
			ProviderEventType: "provider.unknown_fields", ProviderExtension: append(json.RawMessage(nil), event.data...),
		}); err != nil {
			return err
		}
	}
...
func (stream *mockStream) parseStreamChoice(raw json.RawMessage, eventType string) (*pendingFinish, error) {
	...
	if delta.Role != "" {
		if err := stream.enqueue(adapter.NormalizedChunk{
			...
			Role: adapter.MessageRole(delta.Role), ProviderEventType: eventType,
		}); err != nil {
			return nil, err
		}
	}
	...
	if delta.ReasoningContent != "" {
		if err := stream.enqueue(adapter.NormalizedChunk{
			Kind: adapter.ChunkReasoningDelta, ChoiceIndex: choice.Index,
			ReasoningDelta: delta.ReasoningContent, ProviderEventType: eventType,
		}); err != nil {
			return nil, err
		}
	}
...
func (stream *mockStream) enqueueTerminal() error {
	...
	err := stream.enqueue(adapter.NormalizedChunk{
...
func (stream *mockStream) enqueue(chunk adapter.NormalizedChunk) error {
...
func (stream *mockStream) dequeue() adapter.NormalizedChunk {
...
func hasUnknownStreamFields(event []byte, choices []json.RawMessage) bool {
	if !onlyObjectFields(event, "id", "object", "created", "model", "choices", "usage", "system_fingerprint") {
		return true
	}
	for _, rawChoice := range choices {
		if !onlyObjectFields(rawChoice, "index", "delta", "finish_reason") {
			return true
		}
		var choice streamChoice
		if decodeOneJSON(rawChoice, &choice) != nil || !onlyObjectFields(choice.Delta, "role", "content", "reasoning_content", "tool_calls") {
			return true
		}
		var delta streamDelta
...
var _ provideradapter.ChunkStream = (*mockStream)(nil)
