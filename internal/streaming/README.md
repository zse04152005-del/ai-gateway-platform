...
## Mixed pressure and leak gate

`TestStreamingMixedPressureReleasesConnectionsGoroutinesAndBuffers` runs real HTTP/SSE sessions through the shared upstream pool, Mock Adapter decoder, timeout controller, bounded Buffer, failover gate and Usage aggregator. Its deterministic mixed load covers normal completion, 64-Chunk slow clients, randomized post-output cancellation, upstream disconnect without `[DONE]`, and long TTFT. It asserts bounded queue high-water marks, exact clean-EOF versus wrapped-EOF classification, timeout/Usage source facts, cancellation release timestamps, active Handler/connection convergence, Goroutine return and retained-heap allowance. Detailed evidence is in [`2026-07-31-p07-t10-stream-pressure.md`](../../docs/reports/2026-07-31-p07-t10-stream-pressure.md).
