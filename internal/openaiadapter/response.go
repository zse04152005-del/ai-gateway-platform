...
func (openAI *openAIAdapter) ParseResponse(
	ctx context.Context,
	response *http.Response,
) (adapter.NormalizedResponse, error) {
	if openAI == nil || openAI.now == nil {
		return adapter.NormalizedResponse{}, errors.New("openai adapter is not initialized")
	}
	...
	if ctx == nil {
		...
		return adapter.NormalizedResponse{}, errors.New("openai adapter response context must not be nil")
	}
	if err := ctx.Err(); err != nil {
		_ = response.Body.Close()
		return adapter.NormalizedResponse{}, fmt.Errorf("openai adapter response cancelled: %w", err)
	}
	...
	if response.StatusCode != http.StatusOK {
		return adapter.NormalizedResponse{}, openAI.NormalizeError(ctx, response, body)
	}
	...
	if !onlyObjectFields(body, "id", "object", "created", "model", "choices", "usage", "system_fingerprint", "service_tier", "moderation") {
	...
	if envelope.Object != "chat.completion" || envelope.Model == "" {
	...
	if hasValue(envelope.Moderation) {
		return adapter.NormalizedResponse{}, protocolError("parse_response", "moderation_response_unsupported", nil)
	}
	...
	usage, err := optionalUsage(envelope.Usage)
	if err != nil {
		return adapter.NormalizedResponse{}, err
	}
...
func parseChoice(raw json.RawMessage) (adapter.NormalizedChoice, error) {
	if !onlyObjectFields(raw, "index", "message", "finish_reason", "logprobs") {
	...
	if hasValue(choice.Logprobs) {
		return adapter.NormalizedChoice{}, protocolError("parse_choice", "logprobs_unsupported", nil)
	}
	...
	return adapter.NormalizedChoice{
		Index: choice.Index, Message: message,
		FinishReason: finishReason, ProviderFinishReason: providerReason,
	}, nil
}

func parseMessage(raw json.RawMessage) (adapter.Message, error) {
	if !onlyObjectFields(raw, "role", "content", "tool_calls", "refusal", "annotations", "audio", "function_call") {
	...
	if message.Role != string(adapter.RoleAssistant) {
		return adapter.Message{}, protocolError("parse_message", "unexpected_message_role", nil)
	}
	if hasValue(message.Refusal) || hasValue(message.Annotations) || hasValue(message.Audio) || hasValue(message.FunctionCall) {
		return adapter.Message{}, protocolError("parse_message", "unsupported_message_payload", nil)
	}
	normalized := adapter.Message{Role: adapter.RoleAssistant}
...
func optionalUsage(raw json.RawMessage) (*adapter.NormalizedUsage, error) {
	if !hasNonNullValue(raw) {
		return nil, nil
	}
	usage, err := parseUsage(raw, true)
	if err != nil {
		return nil, err
	}
	return &usage, nil
...
func parseUsage(raw json.RawMessage, complete bool) (adapter.NormalizedUsage, error) {
	...
	promptDetails, nestedUnknown, err := parseDetailCounts(fields["prompt_tokens_details"], "/prompt_tokens_details", map[string]string{
		"cached_tokens": "cache_read", "cache_write_tokens": "cache_write", "audio_tokens": "audio_input",
	})
	if err != nil {
		return adapter.NormalizedUsage{}, err
	}
	unmapped = append(unmapped, nestedUnknown...)
	completionDetails, nestedUnknown, err := parseDetailCounts(fields["completion_tokens_details"], "/completion_tokens_details", map[string]string{
		"reasoning_tokens": "reasoning", "audio_tokens": "audio_output",
	})
	if err != nil {
		return adapter.NormalizedUsage{}, err
	}
	...
	usage := adapter.NormalizedUsage{
		...
		CacheReadTokens: promptDetails["cache_read"], CacheWriteTokens: promptDetails["cache_write"],
		ReasoningTokens: completionDetails["reasoning"], AudioInputTokens: promptDetails["audio_input"],
		AudioOutputTokens: completionDetails["audio_output"], Source: adapter.UsageSourceProvider,
		Complete: complete, RawEvidence: evidence, UnmappedFields: unmapped,
	}
...
func parseDetailCounts(
	raw json.RawMessage,
	prefix string,
	mappings map[string]string,
) (map[string]adapter.TokenCount, []string, error) {
	counts := make(map[string]adapter.TokenCount, len(mappings))
	if !hasNonNullValue(raw) {
		return counts, nil, nil
	}
	var fields map[string]json.RawMessage
	if err := decodeOneJSON(raw, &fields); err != nil || fields == nil {
		return nil, nil, protocolError("parse_usage", "invalid_usage_details", err)
	}
	unknown := make([]string, 0)
	for name := range fields {
		target, known := mappings[name]
		if !known {
			unknown = append(unknown, prefix+"/"+escapeJSONPointer(name))
			continue
		}
		count, err := parseTokenCount(fields, name)
		if err != nil {
			return nil, nil, err
		}
		counts[target] = count
...
func hasNonNullValue(raw json.RawMessage) bool {
	trimmed := bytes.TrimSpace(raw)
